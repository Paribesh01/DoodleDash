...
export class GameManager {
    ...
    removeUser(ws: WebSocket) {
        ...
        if (userToRemove) {
            this.games = this.games
                .map(game => {
                    game.players = game.players.filter(player => player.id !== userToRemove.userId);
                    this.notifyAllUsersInRoom(game.roomid, { status: "playerLeft", leftPlayer: userToRemove.userId, game })
            ...
            console.log(`User with ID ${userToRemove.userId} has been removed from the game.`);
        }
    ...
    startGame(userId: string, roomid: string) {
        ...
        if (game) {
            ...
            this.notifyUser(game.players[game.playerTurnIndex].id, { status: "Word", currentWord: game.currentWord })
        ...
        if (game && game.started) {
            if (userId != game.players[game.playerTurnIndex].id) {
                ...
                if (message.toLowerCase() === game.currentWord.toLowerCase()) {
                    ...
                    this.notifyAllUsersInRoom(roomid, { status: 'correctGuess', userId, players: game.players });
                    this.notifyAllUsersInRoom(roomid, { status: "drawing", data: { roomid: roomid, type: "clear" } })
